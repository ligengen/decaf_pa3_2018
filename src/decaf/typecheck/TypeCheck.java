...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitScopy(Tree.Scopy scopy) {
		...
		if(v == null) {
			...
			scopy.symbol = new Variable(".error", BaseType.ERROR, scopy.getLocation());
		} else {
	...
	public void visitForeach(Tree.Foreach foreach) {
		...
		if(foreach.type instanceof Tree.TypeIdent && ((Tree.TypeIdent)foreach.type).typeTag == Tree.UNKNOWN) { // VAR
			...
			if(!((Tree.Ident)_1).type.isArrayType()) {
				...
				//foreach.symbol = new Variable(foreach.var_, ((ArrayType)(((Tree.Ident)_1).type)).getElementType(), foreach.x_loc);
				Symbol v = table.lookup(foreach.var_, true);
				foreach.symbol = (Variable) v;
			}
		}
		if(foreach._while) {
		...
		if(foreach._while) {
			...
			for(int i=1;i<foreach.foreachblock.block.size();i++) {
				foreach.foreachblock.block.get(i).accept(this);
			}
		}
		breaks.pop();
