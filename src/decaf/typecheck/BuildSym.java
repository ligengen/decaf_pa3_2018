...
public class BuildSym extends Tree.Visitor {
	...
	public void visitIdent(Tree.Ident ident) {
		if(ident.var) {
			...
			Variable v = new Variable(ident.name, BaseType.UNKNOWN, ident.loc);
			if(symbol == null) {
				table.declare(v);
			} else {
				if((!table.getCurrentScope().equals(symbol.getScope())) &&
						(!(symbol.getScope().isFormalScope() && table.getCurrentScope().isLocalScope()
	...
	public void visitForeach(Tree.Foreach foreach) {
		//foreach.foreachblock.accept(this);
		foreach.foreachblock.associatedScope = new LocalScope(foreach.foreachblock);
		table.open(foreach.foreachblock.associatedScope);
		foreach.type.accept(this);
		Variable v;
		/*if(foreach.type instanceof Tree.TypeClass) {
			System.out.println("#$%^&*&^%$#@");
		}*/
		if(foreach.type instanceof Tree.TypeIdent) {
			if(((Tree.TypeIdent)foreach.type).typeTag == Tree.UNKNOWN) {
				v = new Variable(foreach.var_, BaseType.UNKNOWN, foreach.x_loc);
				table.declare(v);
			}
			else {
				v = new Variable(foreach.var_, foreach.type.type, foreach.x_loc);
				table.declare(v);
			}
		} else {
			v = new Variable(foreach.var_, foreach.type.type, foreach.x_loc);
		...
		for(Tree s: foreach.foreachblock.block) {
			s.accept(this);
		}
		table.close();
	}
