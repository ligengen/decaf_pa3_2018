...
public class Translater {
	...
	public void genCheckArrayIndex(Temp array, Temp index, Temp default_val, Tree.DynamicAccess dynamicAccess) {
		Temp length = genLoad(array, -OffsetCounter.WORD_SIZE);
		Temp cond = genLes(index, length);
		Label err = Label.createLabel();
		genBeqz(cond, err);
		...
		genBeqz(cond, normal);
		genMark(err);
		dynamicAccess.val = genLoadImm4(0);
		genAssign(dynamicAccess.val, default_val);
		genBranch(exit);
		genMark(normal);
		Temp esz = genLoadImm4(OffsetCounter.WORD_SIZE);
		Temp t = genMul(index, esz);
		Temp base = genAdd(array, t);
		genAssign(dynamicAccess.val, genLoad(base, 0));
		genBranch(exit);
		genMark(exit);
	}
	...
	public void genCheckDivideZero(Temp temp) {
		Label exit = Label.createLabel();
		Temp cond = genEqu(temp, genLoadImm4(0));
		genBeqz(cond, exit);
		Temp msg = genLoadStrConst(RuntimeError.DIVIDE_ZERO);
		genParm(msg);
		genIntrinsicCall(Intrinsic.PRINT_STRING);
		genIntrinsicCall(Intrinsic.HALT);
		genMark(exit);
	...
	public void genCheckRepeatLessThanZero(Temp temp) {
		Label exit = Label.createLabel();
		Temp cond = genLes(temp, genLoadImm4(0));
		genBeqz(cond, exit);
		Temp msg = genLoadStrConst(RuntimeError.REPEAT_LESS_THAN_ZERO);
		genParm(msg);
		genIntrinsicCall(Intrinsic.PRINT_STRING);
		genIntrinsicCall(Intrinsic.HALT);
		genMark(exit);
	...
	public Temp genNewRepeatArray(Temp length, Tree.Expr expr) {
		Temp unit = genLoadImm4(OffsetCounter.WORD_SIZE);
		Temp size = genAdd(unit, genMul(unit, length));
		genParm(size);
		Temp obj = genIntrinsicCall(Intrinsic.ALLOCATE);
		...
		genBeqz(size, exit);
		append(Tac.genSub(obj, obj, unit));
		if(expr.type.isBaseType())
			genStore(expr.val, obj, 0);
		else if(expr.type.isClassType()) {
	...
	public Temp genScopy(Temp src, Class c) {
		Temp size = genLoadImm4(4 * c.vars.size() + 4);
		genParm(size);
		Temp res = genIntrinsicCall(Intrinsic.ALLOCATE);
		for (int i = 0; i < c.vars.size(); i++)
		{
			Temp tmp = genLoad(src, 4 * (i + 1));
			genStore(tmp, res, 4 * (i + 1));
		}
		genStore(genLoadVTable(c.getVtable()), res, 0);
		return res;
	}

	public Temp genX(Temp arr, Temp index) {
		Temp esz = genLoadImm4(OffsetCounter.WORD_SIZE);
		Temp t = genMul(index, esz);
		Temp base = genAdd(arr, t);
		return genLoad(base, 0);
	}
}
